...
auto VulkanRenderer::drawFrame() -> std::expected<void, VulkanError> {
    ...
    VkResult result = vkAcquireNextImageKHR(
        device_, swapChain_, UINT64_MAX, imageAvailable, VK_NULL_HANDLE, &imageIndex);
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || framebufferResized_) {
        ...
        return std::unexpected(VulkanError{
            "Failed to present swap chain image", result});
    }
...
auto VulkanRenderer::querySwapChainSupport(VkPhysicalDevice device) const -> SwapChainSupportDetails {
...
auto VulkanRenderer::createSyncObjects() -> std::expected<void, VulkanError> {
    ...
    for (auto& sync : syncObjects_) {
        if (vkCreateSemaphore(device_, &semaphoreInfo, nullptr, &sync.imageAvailable) != VK_SUCCESS ||
            ...
            return std::unexpected(VulkanError{"Failed to create synchronization objects"});
        }
    }
