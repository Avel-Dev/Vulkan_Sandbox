...
auto VulkanRenderer::initialize(GLFWwindow *window)
...
auto VulkanRenderer::drawFrame() -> std::expected<void, VulkanError> {
  ...
  VkResult result =
      vkAcquireNextImageKHR(device_, swapChain_, UINT64_MAX, imageAvailable,
  ...
  if (auto r = recordCommandBuffer(commandBuffers_[currentFrame_], imageIndex);
      !r) {
    return r;
  }
...
auto VulkanRenderer::createInstance() -> std::expected<void, VulkanError> {
  ...
  if (result != VK_SUCCESS) {
    return std::unexpected(
        VulkanError{"Failed to create Vulkan instance", result});
  }
...
void VulkanRenderer::populateDebugMessengerInfo(
...
auto VulkanRenderer::createSwapChain() -> std::expected<void, VulkanError> {
  ...
  auto indices = findQueueFamilies(physicalDevice_);
  std::uint32_t queueFamilyIndices[] = {*indices.graphicsFamily,
                                        *indices.presentFamily};
...
auto VulkanRenderer::chooseSwapPresentMode(
    const std::vector<VkPresentModeKHR> &modes) const -> VkPresentModeKHR {
...
auto VulkanRenderer::createCommandBuffers()
    -> std::expected<void, VulkanError> {
...
auto VulkanRenderer::recordCommandBuffer(VkCommandBuffer buffer,
                                         std::uint32_t imageIndex)
    -> std::expected<void, VulkanError> {
  ...
  if (vkBeginCommandBuffer(buffer, &beginInfo) != VK_SUCCESS) {
    return std::unexpected(
        VulkanError{"Failed to begin recording command buffer"});
  }
  ...
  for (auto &sync : syncObjects_) {
    if (vkCreateSemaphore(device_, &semaphoreInfo, nullptr,
        ...
        vkCreateSemaphore(device_, &semaphoreInfo, nullptr,
                          &sync.renderFinished) != VK_SUCCESS ||
        vkCreateFence(device_, &fenceInfo, nullptr, &sync.inFlight) !=
            VK_SUCCESS) {
      return std::unexpected(
