...
auto Application::initialize() -> std::expected<void, AppError> {
    ...
    if (auto result = renderer_.initialize(window_); !result) {
        return std::unexpected(AppError{
            "Vulkan initialization failed: " + result.error().message});
    }
...
void Application::run() {
    ...
    renderer_.onWindowResized();  // Triggers device wait
}
...
auto Application::createWindow() -> std::expected<void, AppError> {
    ...
    std::println("Window created: {}x{} (framebuffer: {}x{})",
                 DEFAULT_WIDTH, DEFAULT_HEIGHT, actualWidth, actualHeight);
