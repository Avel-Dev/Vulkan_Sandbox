...
public:
   ...
   */
  [[nodiscard]] auto initialize(GLFWwindow *window)
...
private:
  ...
  void
  populateDebugMessengerInfo(VkDebugUtilsMessengerCreateInfoEXT &info) const;
  ...
  chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &formats) const
      -> VkSurfaceFormatKHR;
  [[nodiscard]] auto
  chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &modes) const
      -> VkPresentModeKHR;
  [[nodiscard]] auto
  ...
  [[nodiscard]] auto recordCommandBuffer(VkCommandBuffer buffer,
